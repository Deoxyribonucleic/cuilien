...
bool vector_adjust_capacity(vector_t* vector, size_t new_size)
{
	...
	if(!vector->data)
	{
		fatal("failed to allocate new memory for vector");
	}
...
void vector_resize(vector_t* vector, size_t new_size)
{
	...
	if(vector_adjust_capacity(vector, new_size) && old_data != 0)
	{
...
void vector_push_back(vector_t* vector, void* new_element)
{
	...
	vector_set(vector, vector->size - 1, new_element);
}
...
void vector_insert(vector_t* vector, void* new_element, size_t at_index)
{
	...
	bool reallocation;
	if(reallocation = vector_adjust_capacity(vector, vector->size + 1))
	{
		memcpy(vector->data, old_data, at_index * vector->element_size);
	}

	// Move remaining elements one index to the right
	memmove(vector_resolve(vector, at_index + 1), old_data + at_index * vector->element_size, (old_size - at_index) * vector->element_size);

	// Insert new element
	vector_set(vector, at_index, new_element);
...
void vector_remove(vector_t* vector, size_t index)
{
	...
	memmove(vector_resolve(vector, index), old_data + (index + 1) * vector->element_size, (old_size - index - 1) * vector->element_size);
